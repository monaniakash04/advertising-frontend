...
const AddNewAds = () => {
  ...
  const { errors, values, handleChange, handleSubmit } = useFormik({
    ...
    onSubmit: async (value) => {
      ...
      formData.append("adpost", file);
      formData.append("heading", value.heading);
      formData.append("body", value.body);
      formData.append("contact", value.phone);
      formData.append("city", value.city);
      formData.append("category", value.category);
      formData.append("payment", 50);
      console.log(value.city, "   ", value.category);

      await axios.post(
        `${process.env.REACT_APP_SERVER_URL}/postAd/userId=${userid}`,
        formData,
        {
